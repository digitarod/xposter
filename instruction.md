...
画像は2通りの指定ができます(混在可・1ツイート最大4枚):

- **画像URL**(例: `https://example.com/item.jpg`)
  → 投稿側が**実行時にダウンロード**して添付。アフィリ商品画像など毎回変わるURLに最適。
- **リポジトリ内パス**(例: `images/a.png`)
  → 固定画像。`images/` に入れてコミットしておく。
...
**GAS** — `postTextToX(text, images)` の第2引数(任意)。URLでもパスでも可:

```javascript
postTextToX('画像つき投稿です', 'https://example.com/item.jpg'); // 画像URL
postTextToX('画像つき投稿です', 'images/a.png');                  // リポジトリ内
postTextToX('テキストだけの投稿');                                // 画像なし

// AffilTweet 互換の例(本文 + アフィリURL + 商品画像URL):
postAffiliate('この商品おすすめ！', 'https://af.example/xxx', 'https://img.example/item.jpg');
```

> 画像URLの取得に失敗した、またはローカル画像が存在しない場合は、警告を出して
> スキップし**テキストのみ投稿**します(投稿自体は失敗しません)。
